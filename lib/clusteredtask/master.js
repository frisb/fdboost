(function() {
  var ClusteredTaskMaster, EventEmitter, async, child_process, path,
  ...
  module.exports = ClusteredTaskMaster = (function(_super) {
    __extends(ClusteredTaskMaster, _super);

    function ClusteredTaskMaster(tr, options, callback) {
      ...
      async.each(workers, configureWorker, function(err) {
        if (err) {
          return callback(err);
        } else {
